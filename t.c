...
int body() // process body function
{
  ...
  while(1){
    ...
    switch(c){
      ...
      case 'x': printList("freeList", freeList); break;
      case 'y': printList("sleepList", sleepList); break;
      case 'z': printList("ReadyList", readyQueue); break;
    }
  }
}/*end body()*/
     ...
     return;
      ...
      if((*queue)->child==NULL){
        ...
        while(current->sibling)
           ...
           current->sibling=p;
      p->sibling=NULL;      printf("Soy %d y Mi hermano es: %d\n",p->pid, current->pid);
      }
}
